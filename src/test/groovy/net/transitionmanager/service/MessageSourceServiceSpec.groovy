 ...
 */
class MessageSourceServiceSpec extends Specification implements DataTest {
    ...
    def setup() {
        ...
        staticMessageSource = applicationContext.getBean(MessageSource)
    }
    ...
    void "test getting an i18n message should return expected value"() {
        setup: 'giving a MessageSourceService'
            MessageSourceService messageSourceService = applicationContext.getBean(MessageSourceService)
            staticMessageSource.addMessage('unit.test.message', Locale.ENGLISH, 'Blah')
    ...
    void "test getting an interpolated i18n message should return the expected value"() {
        setup: 'giving a MessageSourceService'
            MessageSourceService messageSourceService = applicationContext.getBean(MessageSourceService)
            staticMessageSource.addMessage('unit.test.message', Locale.ENGLISH, 'Hello {0}')
