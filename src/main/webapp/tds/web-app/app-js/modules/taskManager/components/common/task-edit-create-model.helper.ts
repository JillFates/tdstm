...
export class TaskEditCreateModelHelper {
	...
	public getModelForEdit(model: any, userTimeZone: string): any {
		...
		this.model.estimatedStart = this.model.estimatedStart ?
			new Date(DateUtils.convertFromGMT(new Date(this.model.estimatedStart), userTimeZone).slice(0, -6)) : ''; // Slice removes the offset from the date string to ensure correct timezone
		this.model.estimatedFinish = this.model.estimatedFinish ?
			new Date(DateUtils.convertFromGMT(new Date(this.model.estimatedFinish), userTimeZone).slice(0, -6)) : '';
