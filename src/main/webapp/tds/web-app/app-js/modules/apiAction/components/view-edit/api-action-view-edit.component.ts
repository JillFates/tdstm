...
export class APIActionViewEditComponent implements OnInit, OnDestroy {
	...
	@ViewChildren('codeMirror') public codeMirrorComponents: QueryList<
		CodeMirrorComponent
	>;
	@ViewChild('apiActionContainer', { static: false })
	apiActionContainer: ElementRef;
	@ViewChild('apiActionViewEditReactionsComponent', { static: false })
	apiActionViewEditReactionsComponent: ApiActionViewEditReactionsComponent;
	...
	protected defaultItem = {
		id: '',
		value: this.translatePipe.transform('GLOBAL.SELECT_PLACEHOLDER'),
	};
	...
	lastSelectedAgentMethodModel: AgentMethodModel = {
		uId: '0',
		dictionaryMethodName: this.translatePipe.transform(
	...
	getAgents(result: any): void {
		...
		if (
			this.apiActionModel.agentMethod &&
			this.apiActionModel.agentMethod.uId
		) {
		...
		this.httpMethodList.push(
			this.translatePipe.transform('GLOBAL.SELECT_PLACEHOLDER')
		);
		...
		if (result && result.data.actionTypes) {
			...
			keys.forEach((key: string) => {
				this.actionTypesList.push({
					id: key,
					name: result.data.actionTypes[key],
				});
			});
		...
		if (result && result.data.remoteCredentialMethods) {
			...
			keys.forEach((key: string) => {
				this.remoteCredentials.push({
					id: key,
					value: result.data.remoteCredentialMethods[key],
				});
			});
		}
	}
		...
		this.apiActionService
			...
			.subscribe(
				(result: any) => {
					if (
						this.modalType === ActionType.CREATE ||
						!this.apiActionModel.credential
					) {
			...
			.subscribe(
				result => {
					...
					if (result.length > 0) {
						...
						if (
							this.apiActionModel.provider &&
							this.apiActionModel.provider.id !== 0
						) {
							this.onProviderValueChange(
								this.apiActionModel.provider,
								true
							);
						}
	...
	getParameters(): void {
		this.apiActionService.getParameters(this.apiActionModel).subscribe(
			(result: any) => {
				...
				setTimeout(() => {
					this.dataParameterListSignature = JSON.stringify(
						this.parameterList
					);
				}, 200);
			},
	...
	protected onSaveApiAction(): void {
		if (this.canSave()) {
			this.apiActionService
				.saveAPIAction(this.apiActionModel, this.parameterList)
				.subscribe(
					(result: any) => {
						if (result) {
							...
							this.apiActionModel.dictionaryMethodName =
								result.dictionaryMethodName;
	...
	public cancelCloseDialog(): void {
		...
		if (
			(this.isDirty() || this.isParameterListDirty()) &&
			this.modalType !== this.actionTypes.VIEW
		) {
			this.promptService
				.open(
				...
				.then(confirm => {
					if (confirm) {
						this.activeDialog.close(
							this.savedApiAction ? this.apiActionModel : null
						);
					} else {
						this.focusForm();
					}
	...
	onDictionaryValueChange(dictionaryModel: DictionaryModel): void {
		dictionaryModel = dictionaryModel
			? dictionaryModel
			: this.defaultDictionaryModel;
		this.apiActionModel.dictionary = dictionaryModel;

		if (
			this.modalType === this.actionTypes.EDIT &&
			this.lastSelectedDictionaryModel &&
			this.lastSelectedDictionaryModel.id !== 0
		) {
			this.prompt
				.open(
					'Confirmation Required',
	...
	private loadDictionaryModel(
		...
		if (changeAgent) {
			if (dictionaryModel.id !== 0) {
				this.apiActionService
					.getActionMethodById(dictionaryModel.id)
					.pipe(takeUntil(this.unsubscribeOnDestroy$))
					.subscribe(
						(result: any) => {
							this.agentMethodList = new Array<
								AgentMethodModel
							>();
							this.agentMethodList.push({
								uId: '0',
							...
							if (this.apiActionModel.agentMethod) {
								this.apiActionModel.agentMethod = result.find(
									agent => {
										return (
											agent.id + agent.name ===
											this.apiActionModel.agentMethod.uId
			...
			if (agent) {
				this.apiActionModel.dictionary = R.clone(
					this.lastSelectedDictionaryModel
				);
			} else {
				this.apiActionModel.dictionary = R.clone(
	...
	onMethodValueChange(event: any): void {
		if (
			this.modalType === this.actionTypes.EDIT &&
			this.lastSelectedAgentMethodModel &&
			this.lastSelectedAgentMethodModel.id !== '0'
		) {
			this.prompt
				.open(
					'Confirmation Required',
	...
	private loadAgentMethodModel(changeMethod: boolean): void {
		if (changeMethod) {
			...
			this.apiActionModel.dictionaryMethodName =
				this.apiActionModel.agentMethod.name ||
				this.apiActionModel.agentMethod.dictionaryMethodName;
			...
			this.lastSelectedAgentMethodModel = R.clone(
				this.apiActionModel.agentMethod
			);
			this.guardParams();
			...
			let agentMethod = this.agentMethodList.find(method => {
				return (
					...
					this.lastSelectedAgentMethodModel.uId
				);
			});
			if (agentMethod) {
				this.apiActionModel.agentMethod = R.clone(
					this.lastSelectedAgentMethodModel
				);
			} else {
				this.apiActionModel.agentMethod = R.clone(
					this.agentMethodList[0]
				);
			}
		}
	}
	...
	private populateReactionScripts(): void {
		...
		APIActionModel.createBasicReactions(
			this.apiActionModel,
			this.apiActionModel.actionType &&
				this.apiActionModel.actionType.id === this.WEB_API
		);
		for (let reactionType in methodScripts) {
			if (methodScripts[reactionType]) {
				let match = this.apiActionModel.eventReactions.find(
	...
	protected onIntervalChange(interval: any, pollingObject: any): void {
		let newVal = DateUtils.convertInterval(
			pollingObject,
			interval.interval
		);
	 ...
	 * @param value
	 */
	protected onProviderValueChange(
		...
		this.providerCredentialList.push({
			id: 0,
			name: this.translatePipe.transform('GLOBAL.SELECT_PLACEHOLDER'),
		});
		this.providerCredentialList = this.providerCredentialList.concat(
			this.agentCredentialList.filter(
				credential =>
					credential.provider &&
					credential.provider.id === providerModel.id
			)
		);
		// Populate only the DataScripts that are related to the provider
		this.providerDatascriptList = new Array<DataScriptModel>();
		this.providerDatascriptList.push({
			id: 0,
			name: this.translatePipe.transform('GLOBAL.SELECT_PLACEHOLDER'),
		});
		this.providerDatascriptList = this.providerDatascriptList.concat(
			this.datascriptList.filter(
				dataScript =>
					dataScript.provider &&
					dataScript.provider.id === providerModel.id
			)
		);
		if (previousValue) {
			this.apiActionModel.defaultDataScript = this.providerDatascriptList.find(
				datascript =>
					datascript.id === this.apiActionModel.defaultDataScript.id
			);
			this.modifySignatureByProperty('defaultDataScript');
			this.apiActionModel.credential = this.providerCredentialList.find(
				credential =>
					credential.id === this.apiActionModel.credential.id
			);
			this.modifySignatureByProperty('credential');
		} else {
	...
	protected onOpenDictionary(): void {
		if (
			this.apiActionModel.dictionary &&
			this.apiActionModel.dictionary.id !== 0
		) {
			this.lastSelectedDictionaryModel = R.clone(
				this.apiActionModel.dictionary
			);
		}
	}
	...
	protected onOpenAgentMethod(): void {
		if (
			this.apiActionModel.agentMethod &&
			this.apiActionModel.agentMethod.uId !== '0'
		) {
			this.lastSelectedAgentMethodModel = R.clone(
				this.apiActionModel.agentMethod
			);
	...
	canSave(): boolean {
		...
		if (this.hasEarlyAccessTMRPermission && actionTypeId === this.WEB_API) {
			return (
				this.simpleInfoForm &&
				this.simpleInfoForm.valid &&
				this.httpAPIForm &&
				this.httpAPIForm.valid &&
				this.validParametersForm &&
				this.validateRequiredFields(this.apiActionModel)
			);
		}
		if (
			this.hasEarlyAccessTMRPermission &&
			actionTypeId !== null &&
			actionTypeId !== this.WEB_API
		) {
			return (
				this.simpleInfoForm &&
				this.simpleInfoForm.valid &&
				this.scriptForm &&
				this.scriptForm.valid &&
				this.validParametersForm &&
				this.validateRequiredFields(this.apiActionModel)
			);
		}
	...
	setSelectEventReaction(type: string, selected: boolean): void {
		const evenReaction = this.apiActionModel.eventReactions.find(
			(item: EventReaction) => item.type === type
		);
		if (evenReaction) {
	...
	getDataScriptName(): string {
		return (
			(this.apiActionModel &&
				this.apiActionModel.defaultDataScript &&
				this.apiActionModel.defaultDataScript.name) ||
			''
		);
	}

	onParametersFormChange(event: {
