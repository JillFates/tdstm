...
class SecurityServiceTests extends Specification {
	...
	@Shared privRoles = ['ADMIN', 'EDITOR', 'USER']
	...
	private void createUnPrivAccount() {		
		...
		unPrivUser = new UserLogin(username:'xyzzy42', password:'guessit', person:unPrivPerson, active:'Y', expiryDate:TimeUtil.nowGMT())
		assert (unPrivUser.validate() && unPrivUser.save())
	}
	...
	def '1 - Test the assignment and removal of roles to a person'() {
	...
	def '2 - Test the getMaxAssignedRole method to see if it is behaving as it should'() {
		...
		when: 'deleting the ADMIN role we can expect that the next role up will be EDITOR'
 			securityService.unassignRoleCode(privPerson, 'ADMIN')
 		then:
	...
	def '3 - Test the getMaxRole method to see that ADMIN is still the top dog'() {

		expect:
			'ADMIN' == securityService.getMaxRole()?.id

	}

	def '4 - Test the getAssignableRoles and getAssignableRoleCodes method'() {
		setup:
	...
	def '6 - Test the hasPermission for different user scenarios'() {
		setup:
			createPrivAccount()
			createUnPrivAccount()
		...
		when: 'called with a null UserLogin it should throw an InvalidParamException exception'
			securityService.hasPermission(null, privPerm)
		then:
			thrown InvalidParamException

		when: 'called with a null permission code it should throw an InvalidParamException exception'
	...
	def '8 - Test the that the password strength logic as tight as Fort Knox'() {
		setup:
