  ...
  describe('Report Links', function(){
    ...
    it('should have expected label and href defined', function(){
      reprotSummaryPage.getReportsList().then(function(list){
        ...
        expect(list[2].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=Login+Badges');
        expect(list[3].getText()).toEqual('Asset Tags');
        expect(list[3].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=Asset+Tag');
        expect(list[4].getText()).toEqual('Transport Worksheets');
        expect(list[4].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=Transportation+Asset+List');
        expect(list[5].getText()).toEqual('Issue Report');
        expect(list[5].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=Issue+Report');
        expect(list[6].getText()).toEqual('Move Results');
        expect(list[6].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=MoveResults');
        expect(list[7].getText()).toEqual('Cabling QA');
        expect(list[7].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=CablingQA');
        ...
        expect(list[9].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=CablingConflict');
        expect(list[10].getText()).toEqual('Cabling Data');
        expect(list[10].getAttribute('href')).toEqual(process.env.BASE_URL+'/tdstm/reports/retrieveBundleListForReportDialog?reportId=CablingData');
