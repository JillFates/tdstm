'use strict';
var AssetsHelper = require('../../support/helpers/assets-helper.js').AssetsHelper;
// var MenuHelper = require('../../support/helpers/menu-helper.js').MenuHelper;
describe('Applpication List', function(){
  ...
  it('go to Application List ', function(){
    assets.goToApplicationList();
  });

  it('click on create app button - should open popup to create de app', function(){
    assets.goToCreateApplicationModal();
  });

  describe('Create Application Modal' , function(){

    it('validate Name Label', function(){
      expect($(assets.nameLabelCss).getText()).toEqual('Name*');
    });

    it('add Name', function(){
      var field = $(assets.nameFieldCss);
      field.sendKeys(appName);
      expect(field.getAttribute('value')).toEqual(appName);
    });

    it('save/close application', function(){
      assets.saveCloseCreateApp();
    });

    it('should be displayed a confirmation message', function(){
      expect($(assets.messageCss).getText()).toEqual('Application App Test 1 created');
    });
  ...
  describe('search for app', function(){
    ...
    it('search by app name', function(){
