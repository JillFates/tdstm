...
class ETLFindSpec extends ETLBaseSpec {
	...
	void 'test can find a domain Property Name with DOMAIN bound instance'() {
		...
		and:
			...
			AssetEntity.executeQuery(_, _, _) >> { String query, Map namedParams, Map metaParams ->
				assetEntities.findAll { it.id == namedParams.id }*.id
			}
			AssetEntity.isAssignableFrom(_) >> { Class<?> clazz->
				return true
			}
		...
		and:
			...
			AssetEntity.executeQuery(_, _, _) >> { String query, Map namedParams, Map metaParams ->
				if (namedParams.containsKey('id')) {
					applications.findAll { it.getId() == namedParams.id && it.project.id == namedParams.project.id }*.id
				} else {
					applications.findAll { it.getAppVendor() == namedParams.appVendor && it.project.id == namedParams.project.id }*.id
				}
			}
	...
	void 'test can trows an Exception if try to use FINDINGS incorrectly based on its results'() {
		...
		and:
			...
			AssetEntity.executeQuery(_, _, _) >> { String query, Map namedParams, Map metaParams ->
				if (namedParams.containsKey('id')){
					return assetEntities.findAll { it.getProject() == GMDEMO && it.id == namedParams.id }*.id
				} else if (namedParams.containsKey('assetName')){
					return assetEntities.findAll { it.getProject() == GMDEMO && it.getAssetName() == namedParams.assetName }*.id
				}
			}
	...
	void 'test can find a domain Property and create the target property automatically with 1 result'() {
		...
		and:
			...
			AssetEntity.executeQuery(_, _, _) >> { String query, Map namedParams, Map metaParams ->
				applications.findAll { it.id == namedParams.id && it.project.id == namedParams.project.id }*.id
			}
