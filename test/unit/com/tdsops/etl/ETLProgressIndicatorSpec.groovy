...
class ETLProgressIndicatorSpec extends ETLBaseSpec {
	...
	void 'test can count number of iterate commands used in an ETL script'() {
		given:
	...
	void 'test can count create a ProgressCallback usinga simple closure'() {
		given:
			def (String fileName, DataSetFacade dataSet) = buildCSVDataSet("""
				name,mfg,model,type
				xraysrv01,Dell,PE2950,Server
	...
	void 'test can report progress using one iterator'() {
		given:
			def (String fileName, DataSetFacade dataSet) = buildCSVDataSet("""
				name,mfg,model,type
				xraysrv01,Dell,PE2950,Server
	...
	void 'test can report progress using more than one iterator'() {
		given:
			def (String fileName, DataSetFacade dataSet) = buildCSVDataSet("""
				name,mfg,model,type
				xraysrv01,Dell,PE2950,Server
