...
class ETLDependencySpec extends ETLBaseSpec {
	...
	@See('TM-13205')
	@ConfineMetaClassChanges([AssetEntity])
	void 'test can add an error when an Asset is dependent on itself while processing Dependencies'() {
		...
		when: 'The ETL script is evaluated'
			etlProcessor.evaluate("""
				...
				iterate {
					domain Application
					extract 'asset' load 'assetName' set assetNameVar
					find Application by 'assetName' eq assetNameVar into 'id'
					set assetVar with DOMAIN
				
					domain Application
					extract 'dependent' load 'assetName' set dependentNameVar
		...
		then: 'Results contains the following values'
			with(etlProcessor.finalResult()) {
				...
				with(domains[0], DomainResult) {
					...
					with(data[0], RowResult) {
						fields.size() == 2
						assertFieldResult(fields['assetName'], 'Exchange 2016', 'Exchange 2016')
						assertFieldResult(fields['id'], null, null)
						with(fields['id'].find, FindResult) {
							query.size() == 1
							assertQueryResult(
								...
								ETLDomain.Application,
								[
									['assetName', FindOperator.eq.name(), 'Exchange 2016'],
								]
							)
						}
					}
				...
				with(domains[1], DomainResult) {
					...
					data.size() == 1
					with(data[0], RowResult) {
						errors == ['An asset dependency reference to itself is prohibited']
						errorCount == 1
						fields.size() == 5
						with(fields['asset'], FieldResult){
							assertFieldResult(it, null, null)
							with(it.find, FindResult) {
								query.size() == 1
								assertQueryResult(
									query[0],
									ETLDomain.Application,
							...
							with(create){
								assetName == 'Exchange 2016'
							}
						}

						with(fields['dependent'], FieldResult){
							assertFieldResult(it, null, null)
						...
						assertFieldResult(fields['type'], 'Runs On', 'Runs On')
						assertFieldResult(fields['status'], 'Unknown', 'Unknown')
						assertFieldResult(fields['dataFlowFreq'], 'Unknown', 'Unknown')
