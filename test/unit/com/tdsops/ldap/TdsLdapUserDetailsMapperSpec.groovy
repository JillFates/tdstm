...
class TdsLdapUserDetailsMapperSpec extends Specification {
    ...
    void "test mapUserFromContext"() {
        given:
        def domain = [debug:true, domains: [TDS: [company: 20, roleMap: [
    ...
    void "test mapUserFromContext no roleMap and defaultRole provided"() {
        given:
        def domain = [
                debug: true,
                domains: [
        ...
        when:
        mapper.mapUserFromContext(context, "foobar2", ldapRoles)
        ...
        1 * mapper.userDetailsService.loadUserByUsername("foobar2")
