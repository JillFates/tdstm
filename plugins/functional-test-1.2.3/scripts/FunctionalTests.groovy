...
target(runFunctionalTests: "The functional test implementation target") {
    ...
    args = argsMap["params"]
    if ((args?.indexOf('http://') >-1) || (args?.indexOf('https://') >-1)) {
        testingBaseURL = args
        // Shift the args
        args = args[1..-1]
    } else {
...
def runTests = {suite, TestResult result, Closure callback ->
    for (TestSuite test in suite.tests()) {
        new File("${testDir}/FUNCTEST-${test.name}.xml").withOutputStream {xmlOut ->
            new File("${testDir}/plain/FUNCTEST-${test.name}.txt").withOutputStream {plainOut ->
                ...
                try {
                    ...
                    new File("${testDir}/FUNCTEST-${test.name}-out.txt").write(outString)
                    new File("${testDir}/FUNCTEST-${test.name}-err.txt").write(errString)
