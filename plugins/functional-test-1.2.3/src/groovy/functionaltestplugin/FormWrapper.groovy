...
class FormWrapper {
    ...
    /**
     * Find form element by name or value
     */
    private handleFormClick(nameOrValue) {
        def f
        def fieldsByName = form.getInputsByName(nameOrValue)
        if (fieldsByName.size() > 1) {
            throw new IllegalArgumentException("Unable to 'click' element named '$nameOrValue', multiple elements with that name found, try clicking by value")
        } else if (fieldsByName.size()) {
            f = fieldsByName[0]
        }
        if (!f) {
            def inputs = form.getInputsByValue(nameOrValue)
            f = inputs.find { it instanceof ClickableElement }
        }
        if (f && (f instanceof ClickableElement)) {
            System.out.println "Clicked [$nameOrValue] which resolved to a [${f.class}]"
            f.click() 
            ...
            throw new IllegalArgumentException("Unable to 'click' element named '$nameOrValue', clickable element not found")
        }
        return
    }

    void click(String nameOrValue) {
        handleFormClick(nameOrValue)
    }
