...
implements WebWindowListener, GroovyInterceptable, 
    ...
    def invokeMethod(String name, args) {
        ...
        if ((name.startsWith('assert') || 
            ...
            try {
                def meth = super.getClass().metaClass.getMetaMethod(name, args)
                if (!meth) {
                    throw new MissingMethodException(name, super.getClass(), args)
                }
                return meth.doMethodInvoke(this, args)
            } catch (Exception e) {
                // Protect against nested func test exceptions when one assertX calls another
                if (!(e instanceof FunctionalTestException)) {
                    throw new FunctionalTestException(this, e)
                } else throw e
            }
        } else {
            def mc = super.getClass().metaClass
    ...
    void webWindowContentChanged(WebWindowEvent event) {
        ...
        if (event.webWindow == mainWindow) {
            newResponseReceived(event.newPage.webResponse)
            _page = event.newPage
            urlStack << [url:event.newPage.webResponse.url, source:'webWindowContentChange event']
        } else {
    ...
    public def byXPath(expr) {
        try {
            return page.getByXPath(expr.toString()).get(0)    
    ...
    public def byName(name) {
        ...
        if (elems.size() > 1) {
            throw new IllegalArgumentException("There is more than one element with [name] attribute [$name]")
        } else if (!elems) {
            return null
        }
        return elems[0]
...
*/    
     ...
     * on the form
     */
    public def form(name, Closure closure) {
        def f
        try {
            f = byId(name)
            if (!f) {
    ...
    protected makeRequest(url, method, paramSetupClosure) {
        ...
        if ((url.indexOf('://') >= 0) || url.startsWith('file:')) {
            ...
            if (url.startsWith('/')) {
                ...
                base = page?.url ? page.url : baseURL
            }
    ...
    protected dumpRequestInfo(reqSettings) {
        ...
        System.out.println('='*40)
        reqSettings?.additionalHeaders?.each {
            System.out.println( "${it.name}: ${it.value}")
        }
        System.out.println('='*40)
    }
	...
	void assertMetaContains(String name, String expected) {
	    ...
	    if (!node) throw new IllegalArgumentException("No meta tag found with name $name")
	...
	void assertMeta(String expected) {
	    def node = page.getElementsByTagName('meta')?.iterator().find { it.attributes?.getNamedItem('name')?.nodeValue == name }
	    if (!node) throw new IllegalArgumentException("No meta tag found with name $name")
        def nodeValue = node.attributes.getNamedItem('content').nodeValue
	    assertEquals stripWS(nodeValue.toLowerCase()), stripWS(expected?.toLowerCase())
	}
