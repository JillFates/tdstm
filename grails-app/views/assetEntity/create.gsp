					...
					<table>
						<tbody>
							<tr>
								...
								<td><input type="text" id="assetName" class="assetName ${config.assetName}" name="assetName" value="${assetEntityInstance.assetName}" tabindex="11" /></td>
							...
							<tr>
								...
								<td><g:select from="${assetTypeOptions}" class="assetType ${config.assetType}" id="assetTypeCreateId" name="assetType" value="${assetType}" onChange="selectManufacturer(this.value,'Create')" tabindex="12" /></td>
								<td class="label" nowrap="nowrap"><label for="priority">Priority</label>
								</td>
								<td ><g:select id="priority" class="priority ${config.priority}" name ="priority" from="${priorityOption}" value= "${assetEntityInstance.priority}" noSelection="${['':' Please Select']}" tabindex="21" />
								</td>
								<td class="label" nowrap="nowrap"><label for="sourceLocationId">Location</label></td>
								<td><input type="text" id="sourceLocationId" class="sourceLocation ${config.sourceLocation}"
									name="sourceLocation" value="${assetEntityInstance.sourceLocation}" size=10 tabindex="31" /></td>
									<td><input type="text" id="targetLocationId" class="targetLocation ${config.targetLocation}"
							...
							<tr>
								<td class="label" nowrap="nowrap"><label for="manufacturer">Manufacturer</label></td>
								 ...
								 <div id="manufacturerCreateId">
								   <g:select id="manufacturer" class="manufacturer ${config.manufacturer}" name="manufacturer.id" from="${manufacturers}" value="${manufacuterer?.id}" onChange="selectModel(this.value,'Create')" optionKey="id" optionValue="name" noSelection="${[null:' Unassigned']}" tabindex="13" />
								 </div>
								</td>
								<td class="label" nowrap="nowrap"><label for="ipAddress">IP1</label></td>
								<td ><input type="text" id="ipAddress" name="ipAddress" class="ipAddress ${config.ipAddress}"
									value="${assetEntityInstance.ipAddress}" tabindex="22" />
								</td>
								<td class="label" nowrap="nowrap"><label for="sourceRoomId">Room</label></td>
								<td><input type="text" id="sourceRoomId" class="sourceRoom ${config.sourceRoom}"
									name="sourceRoom" value="${assetEntityInstance.sourceRoom}" size=10 tabindex="32" /></td>
									<td><input type="text" id="targetRoomId" class="targetRoom ${config.targetRoom}"
							...
							<tr>
								...
								<div id="modelCreateId">
								   <g:select id="model" class="model ${config.model}" name ="model.id" from="${models}" value= "${assetEntityInstance.model}" noSelection="${[null:' Unassigned']}" tabindex="14" 
								...
								<td ><input type="text" id="os" name="os" class="os ${config.os}" value="${assetEntityInstance.os}"  tabindex="24" /></td>
								...
								<td class="rackLabel"><input type="text" id="sourceRackId" class="sourceRack ${config.sourceRack}"
									name="sourceRack" value="${assetEntityInstance.sourceRack}" size=10 tabindex="33" /></td>
								<td class="rackLabel"><input type="text" id="targetRackId" class="targetRack ${config.targetRack}"
									name="targetRack" value="${assetEntityInstance.targetRack}" size=10 tabindex="44" /></td>
								<td class="bladeLabel" style="display: none"><input type="text" id="sourceBladeChassisId" class="sourceBladeChassis ${config.sourceBladeChassis}"
									name="sourceBladeChassis" value="${assetEntityInstance.sourceBladeChassis}" size=10 tabindex="35"/></td>
								<td class="bladeLabel" style="display: none"><input type="text" id="targetBladeChassisId" class="targetBladeChassis ${config.targetBladeChassis}"
									name="targetBladeChassis" value="${assetEntityInstance.targetBladeChassis}" size=10 tabindex="45" /></td>
								<td class="vmLabel" style="display: none"><input type="text" id="virtualHost" class="virtualHost ${config.virtualHost}" name="virtualHost" value="${assetEntityInstance.virtualHost}" size=10 tabindex="37" /></td>
								<td class="vmLabel" style="display: none">&nbsp;</td>
								
							</tr>
							<tr>
								<td class="label" nowrap="nowrap"><label for="shortName">Alt Name</label></td>
								<td ><input type="text" id="shortName" class="shortName ${config.shortName}"
								...
								<td ><input type="text" id="supportType" class="supportType ${config.supportType}" name="supportType"
								...
								<td class="rackLabel"><input type="text" id="sourceRackPositionId" class="sourceRackPosition ${config.sourceRackPosition}"
									name="sourceRackPosition" value="${assetEntityInstance.sourceRackPosition}" size=10 tabindex="34" /></td>
								<td class="rackLabel"> <input type="text" id="targetRackPositionId" class="targetRackPosition ${config.targetRackPosition}"
									name="targetRackPosition" value="${assetEntityInstance.targetRackPosition}" size=10 tabindex="44" /></td>
								<td class="bladeLabel"><input type="text" id="sourceBladePositionId" class="sourceBladePosition ${config.sourceBladePosition}"
									name="sourceBladePosition" value="${assetEntityInstance.sourceBladePosition}" size=10 tabindex="36"/></td>
								<td class="bladeLabel"><input type="text" id="targetBladePositionId" class="targetBladePosition ${config.targetBladePosition}"
							...
							<tr>
								...
								<td ><input type="text" id="serialNumber" class="serialNumber ${config.serialNumber}"
								...
								<td valign="top" class="value ${hasErrors(bean:assetEntityInstance,field:'retireDate','errors')}">
								    <script type="text/javascript" charset="utf-8">
				        				...
				        				</script> <input type="text" class="dateRange retireDate ${config.retireDate}" size="15" style="width: 112px; height: 14px;" name="retireDate" id="retireDate" tabindex="27"
								...
								<td colspan="2"><g:select from="${moveBundleList}" id="moveBundle" class="moveBundle ${config.moveBundle}" name="moveBundle.id" value="${assetEntityInstance.moveBundle}" optionKey="id" optionValue="name" tabindex="38" noSelection="${['':' Please Select']}" />
								</td>
							</tr>
							<tr>
								<td class="label" nowrap="nowrap"><label for="assetTag">Tag</label></td>
								<td ><input type="text" id="assetTag" class="assetTag ${config.assetTag}" name="assetTag" value="${assetEntityInstance.assetTag}" tabindex="17"/></td>
								...
								<script type="text/javascript" charset="utf-8">
					                    jQuery(function($){$('.dateRange').datepicker({showOn: 'both', buttonImage: '${resource(dir:'images',file:'calendar.gif')}', buttonImageOnly: true,beforeShow: customRange});function customRange(input) {return null;}});
					                    </script> <input type="text" class="dateRange maintExpDate ${config.maintExpDate}" size="15" style="width: 112px; height: 14px;" name="maintExpDate" id="maintExpDate" tabindex="28"
								...
								<td colspan="2"><g:select id="planStatus" class="planStatus ${config.planStatus}" name ="planStatus" from="${planStatusOptions}" value= "Unassigned"  noSelection="${['':' Please Select']}" tabindex="39" /></td>
							</tr>
							<tr>
								<td class="label" nowrap="nowrap"><label for="railType">Rail Type</label></td>
								<td ><g:select id="railType" class="railType ${config.railType}" name ="railType" from="${railTypeOption}" value= "${assetEntityInstance.railType}" noSelection="${['':' Please Select']}" tabindex="64" /></td>
								<td class="label" nowrap="nowrap"><label for="truck">Truck/Cart/Shelf</label></td>
								<td ><input type="text" id="truck" class="truck ${config.truck}" name="truck" value="${assetEntityInstance.truck}" size=3 tabindex="61" />
								<input type="text" id="cart" class="cart ${config.cart}" name="cart" value="${assetEntityInstance.cart}" size=3 tabindex="62" />
								<input type="text" id="shelf" class="shelf ${config.shelf}" name="shelf" value="${assetEntityInstance.shelf}" size=2 tabindex="63" /></td>
								<td class="label">Validation</td>
								<td colspan="2">
									<g:select from="${assetEntityInstance.constraints.validation.inList}" id="validation" name="validation" onChange="assetFieldImportance(this.value,'AssetEntity')" value="Discovery"/>
								</td>
							</tr>
