...
<body>
<div class="body task-import-review">
					...
					<div class="col-md-3">
						<ul class="list-group">
							<li class="list-group-item">
								<span class="badge">${results.taskUpdated}</span>
								Tasks Updated:
							</li>
							<li class="list-group-item">
								<span class="badge">${results.taskError}</span>
								Tasks With Errors:
							</li>
						</ul>
		...
		$(document).ready(function() {
			...
			$("#grid").kendoGrid({
				dataSource: {
					type: "json",
					transport: {
						read: "${createLink(action:'importTaskPostResultsData', params:paramsForReviewDataRequest)}"
					},
					error: processErrors,
					schema: {
						model: {
