	...
	<g:form name="issueUpdateForm" controller="task" action="update">
		...
		<input type="hidden" name="room" id="room" value="${assetEntity?.targetRoomName}" />
		<input type="hidden" name="rack" id="rack" value="${assetEntity?.targetRackName}" />
		...
		<table style="margin-left: -2px;">
			...
			<g:if test="${assetComment.assetEntity}">
		   		  <td>Asset:</td><td style="width: 1%">&nbsp;${assetComment?.assetEntity.assetName.encodeAsHTML()}</td>
		   		</g:if>
		   	</tr>
		   	<tr class="prop">
		 	...
		 	<div style="float: left;width: 100%">
				...
				<dl>
	               <g:if test="${assetComment?.assetEntity?.assetType=='Application'}">
		                <dt>Application Name:</dt><dd>&nbsp;${assetComment?.assetEntity.assetName.encodeAsHTML()}</dd>
	                ...
	                <g:elseif test="${assetComment?.assetEntity?.assetType=='Database'}">
	                    <dt>Database Name:</dt><dd>&nbsp;${assetComment?.assetEntity.assetName.encodeAsHTML()}</dd>
	                ...
	                <g:elseif test="${assetComment?.assetEntity?.assetType=='Files'}">
	                    <dt>Storage Name:</dt><dd>&nbsp;${assetComment?.assetEntity.assetName.encodeAsHTML()}</dd>
						<dt>Storage Size:</dt><dd>&nbsp;${assetComment?.assetEntity.size}</dd>
						<dt>Storage Format:</dt><dd>&nbsp;${assetComment?.assetEntity.fileFormat}</dd>
						<dt>Bundle:</dt><dd>&nbsp;${assetComment?.assetEntity.moveBundle}</dd>
	                </g:elseif>
	                <g:else>
						<dt>Asset Tag:</dt><dd>&nbsp;${assetComment?.assetEntity?.assetTag.encodeAsHTML()}</dd>
						<dt>Asset Name:</dt><dd>&nbsp;${assetComment?.assetEntity?.assetName.encodeAsHTML()}</dd>
