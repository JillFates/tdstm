		...
		<table>
			<tbody>
				...
				<tr class="prop">
					...
					<td valign="top" class="value"><tds:hasPermission permission="${Permission.PartyTypeView}"><g:link controller="partyType"
						action="show" id="${partyInstance?.partyType?.id}">${partyInstance?.partyType?.encodeAsHTML()}</g:link></tds:hasPermission>
					<tds:hasPermission permission="${Permission.PartyView}">${partyInstance?.partyType?.encodeAsHTML()}</tds:hasPermission>
					</td>
