...
/**
	 ...
	 * @controllerMethod
	 */
	Map processFileUpload(session, Object request, UserLogin byWhom, Project project, Map formOptions) {
		if (! securityService.hasPermission(byWhom, 'GenerateTasks', true)) {
	...
	private String saveImportSpreadsheet(Object request, UserLogin byWhom, String paramName) {
		...
		String filename = "TaskImport-${byWhom.id}-" + com.tdsops.common.security.SecurityUtil.randomString(10)+'.xls'

		// Save file locally
		String fqfn=getFilenameWithPath(filename)
		log.info "saveImportSpreadsheet() user $byWhom uploaded TaskImport spreadsheet to $fqfn"
	...
	/**
	 * Used to read the Task Import Spreadsheet and load up a list of task properties. This will
	 * iterate over the taskSpreadsheetColumnMap Map to pluck the values out of the appropriate columns of
	...
	/**
	 * Used to assign the correct icon to the individual tasks based on the state of the task
	 * @param tasks - the list of task maps
	 */
	private void setIconsOnTasks(List tasks) {
		for (int i=0; i < tasks.size() ; i++) {
			String icon = 'pencil.png'
			if (tasks[i].errors) {
				icon = 'exclamation.png'
			}
			tasks[i].icon = HtmlUtil.resource([dir: 'icons', file: icon, absolute: false])
		}
	}

	/**
	 * Used to set the appropriate property on the task Map to indicate that a defaulted value
	 ...
	 * @controllerMethod
	 */
	Map generateModelForReview(session, UserLogin byWhom, Project project, Map formOptions) {
	 ...
	 */
	void deletePreviousUpload(UserLogin byWhom, Map formOptions) {
		if (!securityService.hasPermission(byWhom, 'GenerateTasks', false)) {
			securityService.reportViolation("attempted to delete an uploaded task import spreadsheet(${formOptions.filename})", byWhom)
	 ...
	 * @controllerMethod
	 */
	List generateReviewData(session, UserLogin byWhom, Project project, String filename, Map formOptions) {
		if (! securityService.hasPermission(byWhom, 'GenerateTasks', true)) {
			throw new UnauthorizedException('Do not have the required permission for to import task information')
		}
		...
		List tasks = validateSpreadsheetContent(byWhom, project, workbook, sheetInfoOpts, formOptions)
	...
	private List<Map> validateSpreadsheetContent(UserLogin byWhom, Project project, Workbook workbook, Map sheetInfoOpts, Map formOptions) {
		...
		validateUploadedTasks(byWhom, tasks, project, sheetInfoOpts, formOptions)
	 ...
	 */
	private void validateUploadedTasks(UserLogin byWhom, List<Map> tasks, Project project, Map sheetInfoOpts, Map formOptions) {
		def allTeamCodes = partyRelationshipService.getTeamCodes()
		allTeamCodes << 'STAFF'
		def moveEvents = MoveEvent.findAllByProject(project)

		for (int i=0; i < tasks.size(); i++) {
			if(!StringUtil.isBlank(tasks[i].responsibleResource)){
				def personInfo = personService.findPersonByFullName(tasks[i].responsibleResource)
				if(personInfo.isAmbiguous){
					tasks[i].errors << 'Found multiple people by name'
				}	
			}
			if(!StringUtil.isBlank(tasks[i].team)){
				def team = allTeamCodes.find{it == tasks[i].team}
				if(!team){
					tasks[i].errors << "Team doesn't exist."
				}	
			}
			

			if(!StringUtil.isBlank(tasks[i].moveEvent)){
				def event = moveEvents.find{it.name == tasks[i].moveEvent}
				if(!event){
					tasks[i].errors << "Move Event doesn't exist."	
				}	
			}
		}
	 ...
	 */
	@Transactional
	Map postChangesToTasks(session, UserLogin byWhom, Project project, Object formOptions) {
		if (! securityService.hasPermission(byWhom, 'GenerateTasks', true)) {
