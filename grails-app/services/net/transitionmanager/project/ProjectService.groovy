...
class ProjectService implements ServiceMethods {
	...
	def getprojectEditDetails(Project projectInstance){
		...
		def partnerStaff
		def projectCompany = PartyRelationship.find("from PartyRelationship p where p.partyRelationshipType = 'PROJ_COMPANY' and p.partyIdFrom = $projectInstance.id and p.roleTypeCodeFrom = '$RoleType.CODE_PARTY_PROJECT' and p.roleTypeCodeTo = '$RoleType.CODE_PARTY_COMPANY' ".toString())
		...
		def moveManager = PartyRelationship.find("from PartyRelationship p where p.partyRelationshipType = 'PROJ_STAFF' and p.partyIdFrom = $projectInstance.id and p.roleTypeCodeFrom = '$RoleType.CODE_PARTY_PROJECT' and p.roleTypeCodeTo = '$RoleType.CODE_TEAM_MOVE_MGR' ".toString())
		def companyStaff = PartyRelationship.findAll("from PartyRelationship p where p.partyRelationshipType = 'STAFF' and p.partyIdFrom = $projectCompany.partyIdTo.id and p.roleTypeCodeFrom = '$RoleType.CODE_PARTY_COMPANY' and p.roleTypeCodeTo = '$RoleType.CODE_PARTY_STAFF' order by p.partyIdTo".toString())
		companyStaff.each {
		...
		def clientStaff = PartyRelationship.findAll("from PartyRelationship p where p.partyRelationshipType = 'STAFF' and p.partyIdFrom = $projectInstance.client.id and p.roleTypeCodeFrom = '$RoleType.CODE_PARTY_COMPANY' and p.roleTypeCodeTo = '$RoleType.CODE_PARTY_STAFF' order by p.partyIdTo".toString())
			clientStaff.each {
			if (it.partyIdTo?.lastName == null) {
				it.partyIdTo?.lastName = ""
			}
