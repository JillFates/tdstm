 ...
 * @author Diego Scarpa
 */
class EmailDispatchService implements ServiceMethods, EventBusAware {
    ...
    static Map<String, String> EMAIL_TEMPLATES = [
            ...
            etlProcessFailed   : "/auth/_etlProcessFailedEmail"
    ].asImmutable()
        ...
        switch (ed.bodyTemplate) {
            ...
            case "adminResetPassword":
                return [
                        ...
                        supportEmail    : "support@transitionaldata.com",
                        username        : ed.toPerson.userLogin?.username]
            case "batchPostingResults":
                return [
                        person   : ed.toPerson.firstName,
                        batches  : emailParams.batches,
                        serverURL: serverURL
                ]
            case "etlProcessFailed":
                return emailParams + [
                        person   : ed.toPerson.firstName,
                        serverURL: serverURL
                ]
    ...
    /**
     * If {@link net.transitionmanager.imports.DataTransformService#transformEtlData(net.transitionmanager.security.UserLogin, java.lang.Long, java.lang.Long, java.lang.String, java.lang.Boolean)}
     * failed and {@link DataScript#isAutoProcess} is true, it is necessary to send an email with
     * @param emailDispatchParams
     */
    @Transactional
    @Subscriber(ETLTransformImportFailureEvent.EVENT_NAME)
    void sendEmailForETLTransformationFailure(ETLTransformImportFailureEvent event) {

		log.info "Event for ETL transformation failure:$event"

        Person whom = Person.get(event.personId)
        DataScript dataScript = DataScript.get(event.dataScriptId)

        String paramsJson = ([
                projectName   : dataScript.project.name,
                providerName  : dataScript.provider.name,
                dataScriptName: dataScript.name,
                groupGuid     : event.groupGuid,
                failedDuring  : event.failureDuring,
                errorMessage  : event.errorMessage
        ] as JSON).toString()

        EmailDispatch emailDispatch = new EmailDispatch()
        emailDispatch.origin = EmailDispatchOrigin.TASK
        emailDispatch.subject = 'ETL Process Failed'
        emailDispatch.bodyTemplate = 'etlProcessFailed'
        emailDispatch.paramsJson = paramsJson
        emailDispatch.toAddress = whom.email
        emailDispatch.toPerson = whom
        emailDispatch.createdBy = whom

        if (emailDispatch.save(failOnError: false)) {
            createEmailJob(emailDispatch, [person: emailDispatch.createdBy])
        } else {
            //TODO: dcorrea. Review this with John!
        }
    }
}
