	...
	*/
	...
	def deleteTaskData(def moveEvent, def deleteManual=false) {
		...
		try {
			...
			def andClause = deleteManual ? '' : 'AND t.autoGenerated=true'
			def taskList = AssetComment.findAll("FROM AssetComment t WHERE t.moveEvent.id=:meId AND t.taskBatch IS NULL $andClause", [meId:moveEvent.id])
			if (taskList.size() > 0) {
				def ids = taskList*.id
				depDeleted = TaskDependency.executeUpdate("DELETE FROM TaskDependency td WHERE (td.predecessor IN (:tasks) OR td.assetComment IN (:tasks))", [tasks:taskList])
				notesDeleted = CommentNote.executeUpdate('DELETE FROM CommentNote n WHERE n.assetComment.id IN (:ids)', [ids:ids])
				taskDeleted = AssetComment.executeUpdate("DELETE FROM AssetComment t WHERE t.id IN (:ids) AND t.taskBatch.id IS NULL $andClause", [ids:ids])
			}
			msg = "Deleted $taskDeleted tasks, $depDeleted dependencies and $notesDeleted task notes"
		} catch(e) {
	...
	def getTasksOfBatch(taskBatchId, person, project) {
		...
		return assetComments.collect({ assetComment ->
			return [
				...
				'team': assetComment.role?assetComment.role:'',
				'person': assetComment.assignedTo?assetComment.assignedTo.toString():'',
...
log.info "tasksCount=$tasksCount, timeAsOf=$timeAsOf, planStartTime=$planStartTime, planCompTime=$planCompTime, tasksCompleted=$tasksCompleted, remainingStepTime=$remainingStepTime, planDuration=$planDuration, planTaskPace=$planTaskPace, tasksRemaining=$tasksRemaining, " +
	...
	def assignToSelectHtml(projectId, taskId, defaultId, elementId) {
		...
		def list = []
		projectStaff.each {
			def roleTypeName = "${it.role}"

			list << [ id:it.staff.id,
				nameRole:"${roleTypeName}: ${it.staff.toString()}",
				sortOn:"${roleTypeName},${it.staff.firstName} ${it.staff.lastName}"
			]
		}
		list.sort { it.sortOn }
