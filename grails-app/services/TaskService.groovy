		...
		try {
			...
			if ( recipeGroups ) {
				...
				recipeGroups.each() { g -> 
					...
					groups[g.name] = findAllAssetsWithFilter(moveEvent, g, groups, exceptions)
								...
								taskSpecs.each() { ts -> 
									if (ts instanceof String) {
										if (ts.isNumber()) {
											...
											throw new RuntimeException("Task Spec (${taskSpec.id}) 'predecessor.taskSpec' value ($ts) must be numeric")
										}
									}
