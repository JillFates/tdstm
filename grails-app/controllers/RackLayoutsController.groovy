...
class RackLayoutsController implements ControllerMethods {
	...
	private retrieveRackLayout(paramsMap) {
		...
		asset.each {
			...
			if (it.asset) {
				...
				if (location == 1) {
					queryParams.roomName = assetEntity.roomSource?.roomName
					queryParams.rackName = assetEntity.rackSource?.tag
				}
				else {
					srcTrg = 'Target'
					queryParams.roomName = assetEntity.roomTarget?.roomName
					queryParams.rackName = assetEntity.rackTarget?.tag
				}
				...
				if (fullRows.contains(i)) {
					...
					if (!printView) {
						if (assetDetails.asset.source == 1) {
							...
							assetLocation = assetEntity.roomSource?.location
						}
						else {
							assetRoom = assetEntity.roomTarget?.id
							assetLocation = assetEntity.roomTarget?.location
						}
	...
	def retrieveCablingDetails() {
		...
		assetCableMapList.each {
			...
			if (it.assetLoc == 'S') {
				def sourceRack = it.assetFrom?.rackSource?.tag
				title += sourceRack ? " ( " + sourceRack + " / " + it.assetFrom?.sourceRackPosition + " )" : ""
			}
			else {
				title += " ( " + it.assetFrom?.rackTarget?.tag + " / " + it.assetFrom?.targetRackPosition + " )"
			}
			assetCablingDetails << [
