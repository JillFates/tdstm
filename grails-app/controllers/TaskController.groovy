...
"""
		...
		try {
			...
			redirect(controller:"task", action:"taskGraph", params:[uri:uri, fileName:uri.split('/')[uri.split('/').size()-1]])
		} catch(e) {
...
"""
	...
	/**
	 * Used to display the task graph with zooming and panning
	 * @param uri the uri pointing to the svg file
	 * @return data the svg data from the file generated by graphiz
	 */
	def taskGraph = {
		String fileContents = new File(grailsApplication.config.graph.targetDir + params.fileName).text
		return [svg:fileContents]
	}
