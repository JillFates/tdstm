		...
		try {
			...
			if (securityService.loggedIn) {
				...
				def moveEventProjectClientStaff = PartyRelationship.find(
					...
					"and p.partyIdFrom = ${moveObject?.project?.client?.id} and p.roleTypeCodeFrom = '$RoleType.CODE_COMPANY' " +
					"and p.roleTypeCodeTo = '$RoleType.CODE_STAFF' and p.partyIdTo = $securityService.currentPersonId")
