...
class ProjectController implements ControllerMethods {
	...
	def save() {
		...
		Project.withTransaction { status ->
			...
			if (projectManagerId > 0) {
				personService.addToProjectTeam(project.id.toString(), projectManagerId.toString(), RoleType.CODE_PROJ_MGR)
			}
	...
	def fetchStaffList() {
		...
		String staffRole = request.JSON.role ?: RoleType.CODE_STAFF
		List<Map> staffList = []
		String staffQuery = "FROM PartyRelationship p \
			WHERE p.partyRelationshipType = 'STAFF' AND p.partyIdFrom = :company \
			AND p.roleTypeCodeFrom.id = '$RoleType.CODE_COMPANY' AND p.roleTypeCodeTo.id = :staffRole"
