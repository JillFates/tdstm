	...
	def showComment() {
		...
		// TODO : Security : Should reduce the person objects (create,resolved,assignedTo) to JUST the necessary properties using a closure
			...
			commentList << [
				...
				etFinish               : etFinish,
				etStart                : etStart,
				eventList              : eventList.collect { [id: it.id, name: it.name] },
	...
	*/
	...
	def applicationArchitectureGraph() {
		...
		if ((context.assetId == null || context.assetId == -1) || (context.levelsUp == null || context.levelsDown == null)) {
			render(view: '_applicationArchitectureGraph', model: architectureGraphService.architectureGraphModel(context.assetId, context.levelsUp, context.levelsDown, context.mode))
			return
		}
