...
databaseChangeLog = {
	...
	changeSet(author: "tpelletier", id: "20190627 TM-13321-6") {
		...
		sql('''UPDATE party_relationship 
                       SET role_type_code_from_id = SUBSTRING(role_type_code_from_id,6)
                       WHERE role_type_code_from_id like "ROLE_%";''')
	}

	changeSet(author: "tpelletier", id: "20190627 TM-13321-7") {
		comment('Rolling back fixing party role prefix ROLE_.')
		sql('''UPDATE party_relationship
	                       SET role_type_code_to_id = SUBSTRING(role_type_code_to_id,6)
	                       WHERE role_type_code_to_id like "ROLE_%";''')
	}
	...
	changeSet(author: "tpelletier", id: "20190627 TM-13321-9") {
		comment('Rolling back role prefix ROLE_.')
		sql('''UPDATE role_type  SET type = SUBSTRING(type,6) WHERE type = 'ROLE_SECURITY' AND type like "ROLE_%";''')
	}
}
